...
export function RegisterPage() {
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    void (async () => {
      ...
      setNotice(
        "Аккаунт создан. Если в проекте включено подтверждение email — проверьте почту. Иначе можно сразу войти.",
      );
    })();
  };
  ...
  return (
    <div className="wb-page wb-page--auth">
      <h1 className="wb-h1">Регистрация</h1>
      ...
      <form onSubmit={onSubmit} className="wb-form">
        ...
        <label className="wb-form-label">
          <span>Пароль</span>
          <PasswordInput autoComplete="new-password" required minLength={6} value={password} onChange={setPassword} />
        </label>
        {error && (
        ...
        {notice && <p className="wb-status-ok">{notice}</p>}
