...
| Дата       | Изменения |
|------------|-----------|
| 2026-04-23 | Надёжность продакшена: `getSession` с `catch`/`finally` (не зависает на ошибке сети); загрузка `user_state` с таймаутом 25 с и понятной ошибкой; trim у Supabase URL/ключа. |
